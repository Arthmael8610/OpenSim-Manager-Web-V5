...
//	echo Affichage_Entete($_SESSION['opensim_select']);
	...
	$reqA = $mysqli->query($sqlA) or die('Erreur SQL !<br>'.$sqlA.'<br>'.$mysqli->error());
	$dataA = $reqA->fetch_assoc();
...
//*******************************************************************************
	if ($_POST["RAZ_LISTE_OBJ"]){
		...
		$req0 = $mysqli->query($sql0) or die('Erreur SQL !<br>'.$sql0.'<br>'.$mysqli->error());
