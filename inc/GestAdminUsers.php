...
if (isset($_SESSION['authentification']) && $_SESSION['privilege']>= 3)
{
	...
	if (isset($_POST['cmd']))
	{
		...
		if ($_POST['cmd'] == 'Change')
		{
		    ...
		    {	
                ...
                ";	
		        $reqUp = $mysqli->query($sqlUp) or die('Erreur SQL !<p>'.$sqlUp.'</p>'.$mysqli->error());
		...
		if ($_POST['cmd'] == 'Enregistrer')
		{
            ...
            ";
			$reqIns = $mysqli->query($sqlIns) or die('Erreur SQL !<p>'.$sqlIns.'</p>'.$mysqli->error());
    ...
    {	
        $sqlDel = "
            ...
            AND `lastname` = '".$_POST['oldLastName']."' 
        ";	
        $reqDel = $mysqli->query($sqlDel) or die('Erreur SQL !<p>'.$sqlDel.'</p>'.$mysqli->error());
    ...
    if ($_POST['cmd'] != 'Ajouter')
    {
        ...
        while ($data = $req->fetch_assoc())
        {
            ...
            if ($data['privilege'] == 1)
            {
                ...
                $req1 = $mysqli->query($sql1) or die('Erreur SQL !<p>'.$sql1.'</p>'.$mysqli->error());
